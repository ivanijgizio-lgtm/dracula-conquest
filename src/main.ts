...
interface Province {
  id: number;
  name: string;
  owner: 'player' | 'ai' | 'werewolf' | null;
  x: number;
  y: number;
  aiGarrison: Army;
  playerGarrison?: Army;
  siegeBy: 'player' | 'ai' | 'werewolf' | null;
  neighbors: number[];
  buildings: Building[];
  income: number;
  support: Support;
...
interface PlayerState {
  ...
  gold: number;
  blood: number;
  lords: { name: string; battles: number }[];
  mobileArmy: Army;
  techs: { militaryReform: boolean; necromancy: boolean; tradeRoutes: boolean };
  marketUsed: boolean;
  allianceWithAI: boolean;
  truceTurnsAI: number;
  truceTurnsWolf: number;
  hasCitadel: boolean;
  hasSoulCollector: boolean;
}
interface GameState {
  turn: number;
  day: number;
  ...
  surrenderActive: boolean;
  armyBattleActive: boolean;
  tutorialStep: number;
  fogOfWar: boolean;
  selectedProvinceId: number | null;
  pendingActionProvId: number | null;
  enemyArmyTarget: 'ai' | 'werewolf' | null;
  weather: { lightning: boolean; rain: boolean; sunset: boolean };
  player: PlayerState;
  ai: { gold: number; blood: number; generals: { inquisitor: number }; mobileArmy: Army; faith: number };
  werewolf: { gold: number; blood: number; generals: { alpha: number }; mobileArmy: Army };
  provinces: Province[];
}
...
// ================= ЛОР И ЭНЦИКЛОПЕДИЯ =================
const BUILD_LORE: Record<string, string> = {
  'build': "🏗️ СТРОИТЬ: Возводите тёмные сооружения, усиливающие вашу мощь...",
  'recruit': "🧛 ПРИЗВАТЬ: Найдите подходящих солдат...",
  'garrison': "🛡️ ГАРНИЗОН: Перемещайте войска...",
  'cemetery': "🪦 Кладбище: Дарует +5 крови за ход.",
  'barracks': "⚔️ Казармы Lv1: Без них войска не могут быть призваны.",
  'barracks_lv2': "⚔️⬆️ Казармы Lv2: Открывает призыв Рыцарей Тьмы.",
  'ritual': "🕯️ Храм Тьмы: +5 поддержки, +3 лояльности. Открывает найм Лордов.",
  'dungeon': "⛓️ Тюрьма: +10 поддержки, -5 лояльности.",
  'executions': "🪓 Казни: +15 поддержки, -10 лояльности, -200 населения.",
  'ball': "🎭 Бал Вампиров: +20 поддержки, +5 лояльности, -200 населения.",
  'center': "🧛 Центр Обращения: +10 поддержки, +5 лояльности, +100 населения.",
...
const LORD_NAMES: string[] = ["Граф Дракулос", "Леди Сильвана", "Барон Ноктюрн", "Графиня Морвен", "Владыка Варгос", "Лорд Мортис", "Принц Теней", "Леди Вэйн", "Генерал Кровавый Клык", "Некромант Зерет"];
...
function getDefaultGame(): GameState {
  return {
    ...
    provinces: [
      ...
      { id: 6, name: 'Молдавия', owner: 'ai', x: 630, y: 260, aiGarrison: { infantry: 5, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [3, 5, 7, 12, 13], buildings: [], income: 2, support: { player: 40, ai: 40, werewolf: 20 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'forest', terrainBonus: 2, loyalty: 70 },
      { id: 7, name: 'Одесса', owner: 'ai', x: 660, y: 340, aiGarrison: { infantry: 5, archer: 3, cavalry: 1 }, siegeBy: null, neighbors: [5, 6, 12, 13], buildings: [], income: 1, support: { player: 45, ai: 30, werewolf: 25 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 8, name: 'Богемия', owner: 'ai', x: 430, y: 250, aiGarrison: { infantry: 10, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [3, 4, 9, 24], buildings: [{type:'church', lvl:1}], income: 2, support: { player: 20, ai: 70, werewolf: 10 }, population: 2500, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 9, name: 'Саксония', owner: 'ai', x: 320, y: 210, aiGarrison: { infantry: 5, archer: 1, cavalry: 1 }, siegeBy: null, neighbors: [1, 2, 8, 14], buildings: [], income: 2, support: { player: 30, ai: 60, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 10, name: 'Сербия', owner: 'ai', x: 520, y: 370, aiGarrison: { infantry: 5, archer: 0, cavalry: 5 }, siegeBy: null, neighbors: [4, 11, 25], buildings: [], income: 1, support: { player: 50, ai: 30, werewolf: 20 }, population: 1000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 50 },
      { id: 11, name: 'Болгария', owner: 'ai', x: 580, y: 420, aiGarrison: { infantry: 10, archer: 5, cavalry: 2 }, siegeBy: null, neighbors: [5, 10, 13, 15, 29], buildings: [], income: 1, support: { player: 40, ai: 40, werewolf: 20 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'mountains', terrainBonus: 5, loyalty: 50 },
      { id: 12, name: 'Киевская Русь', owner: 'ai', x: 720, y: 180, aiGarrison: { infantry: 10, archer: 1, cavalry: 1 }, siegeBy: null, neighbors: [6, 7, 13, 35], buildings: [], income: 1, support: { player: 15, ai: 80, werewolf: 5 }, population: 1800, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      ...
      { id: 14, name: 'Польша', owner: 'ai', x: 370, y: 110, aiGarrison: { infantry: 10, archer: 3, cavalry: 5 }, siegeBy: null, neighbors: [8, 9, 39, 16], buildings: [{type:'church', lvl:1}], income: 2, support: { player: 10, ai: 80, werewolf: 10 }, population: 2500, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      ...
      { id: 18, name: 'Босния', owner: 'ai', x: 450, y: 310, aiGarrison: { infantry: 10, archer: 1, cavalry: 1 }, siegeBy: null, neighbors: [17, 21, 25, 27], buildings: [], income: 2, support: { player: 25, ai: 35, werewolf: 40 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 19, name: 'Ломбардия', owner: 'werewolf', x: 220, y: 130, aiGarrison: { infantry: 10, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [1, 20, 16], buildings: [], income: 1, support: { player: 10, ai: 20, werewolf: 70 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'mountains', terrainBonus: 5, loyalty: 60 },
      { id: 20, name: 'Карпаты', owner: 'werewolf', x: 480, y: 150, aiGarrison: { infantry: 15, archer: 1, cavalry: 1 }, siegeBy: null, neighbors: [21, 22, 3], buildings: [], income: 1, support: { player: 10, ai: 10, werewolf: 80 }, population: 3000, slaveIncome: 0, fortification: 0, terrain: 'forest', terrainBonus: 2, loyalty: 70 },
      { id: 21, name: 'Дикая пуща', owner: 'werewolf', x: 560, y: 170, aiGarrison: { infantry: 10, archer: 1, cavalry: 5 }, siegeBy: null, neighbors: [20, 22, 5, 18, 27], buildings: [], income: 1, support: { player: 10, ai: 10, werewolf: 80 }, population: 2500, slaveIncome: 0, fortification: 0, terrain: 'forest', terrainBonus: 2, loyalty: 60 },
      { id: 22, name: 'Пруссия', owner: 'ai', x: 560, y: 90, aiGarrison: { infantry: 10, archer: 1, cavalry: 1 }, siegeBy: null, neighbors: [20, 21, 39], buildings: [], income: 2, support: { player: 15, ai: 75, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 23, name: 'Германия', owner: 'ai', x: 310, y: 100, aiGarrison: { infantry: 15, archer: 3, cavalry: 3 }, siegeBy: null, neighbors: [14, 38, 22], buildings: [], income: 2, support: { player: 20, ai: 70, werewolf: 10 }, population: 2500, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 24, name: 'Бавария', owner: 'ai', x: 370, y: 220, aiGarrison: { infantry: 12, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [2, 9, 16, 8], buildings: [], income: 2, support: { player: 20, ai: 70, werewolf: 10 }, population: 2500, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 25, name: 'Греция', owner: 'ai', x: 540, y: 440, aiGarrison: { infantry: 10, archer: 5, cavalry: 1 }, siegeBy: null, neighbors: [10, 11, 4, 18], buildings: [], income: 2, support: { player: 30, ai: 50, werewolf: 20 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'mountains', terrainBonus: 5, loyalty: 50 },
      { id: 26, name: 'Балканы', owner: 'ai', x: 600, y: 380, aiGarrison: { infantry: 8, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [5, 11, 25], buildings: [], income: 2, support: { player: 40, ai: 40, werewolf: 20 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 27, name: 'Словакия', owner: 'ai', x: 490, y: 230, aiGarrison: { infantry: 8, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [3, 18, 21, 8], buildings: [], income: 1, support: { player: 30, ai: 60, werewolf: 10 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 28, name: 'Моравия', owner: 'ai', x: 450, y: 150, aiGarrison: { infantry: 8, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [3, 14, 20, 27], buildings: [], income: 1, support: { player: 20, ai: 70, werewolf: 10 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 30, name: 'Франция', owner: 'ai', x: 170, y: 180, aiGarrison: { infantry: 20, archer: 5, cavalry: 3 }, siegeBy: null, neighbors: [31, 32, 1, 33, 34], buildings: [{type:'church', lvl:1}], income: 4, support: { player: 10, ai: 80, werewolf: 10 }, population: 3000, slaveIncome: 0, fortification: 2, terrain: 'plains', terrainBonus: 0, loyalty: 80 },
      { id: 31, name: 'Бретань', owner: 'ai', x: 110, y: 160, aiGarrison: { infantry: 10, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [30, 33, 36], buildings: [], income: 2, support: { player: 15, ai: 75, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 32, name: 'Бургундия', owner: 'ai', x: 220, y: 210, aiGarrison: { infantry: 12, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [30, 16, 1], buildings: [], income: 2, support: { player: 20, ai: 70, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 33, name: 'Аквитания', owner: 'ai', x: 150, y: 250, aiGarrison: { infantry: 12, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [30, 31, 34], buildings: [], income: 2, support: { player: 20, ai: 70, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 34, name: 'Испания', owner: 'ai', x: 130, y: 330, aiGarrison: { infantry: 15, archer: 3, cavalry: 5 }, siegeBy: null, neighbors: [30, 33, 36, 37], buildings: [{type:'church', lvl:1}], income: 3, support: { player: 10, ai: 80, werewolf: 10 }, population: 2500, slaveIncome: 0, fortification: 2, terrain: 'plains', terrainBonus: 0, loyalty: 80 },
      { id: 36, name: 'Португалия', owner: 'ai', x: 80, y: 300, aiGarrison: { infantry: 10, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [31, 34, 37], buildings: [], income: 2, support: { player: 20, ai: 70, werewolf: 10 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 37, name: 'Англия', owner: 'ai', x: 70, y: 130, aiGarrison: { infantry: 15, archer: 5, cavalry: 3 }, siegeBy: null, neighbors: [36, 40], buildings: [{type:'church', lvl:1}], income: 4, support: { player: 10, ai: 80, werewolf: 10 }, population: 3000, slaveIncome: 0, fortification: 2, terrain: 'plains', terrainBonus: 0, loyalty: 80 },
      { id: 38, name: 'Шотландия', owner: 'ai', x: 60, y: 70, aiGarrison: { infantry: 10, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [37], buildings: [], income: 2, support: { player: 15, ai: 75, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 1, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 39, name: 'Ирландия', owner: 'ai', x: 20, y: 110, aiGarrison: { infantry: 8, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [37], buildings: [], income: 1, support: { player: 20, ai: 70, werewolf: 10 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 40, name: 'Дания', owner: 'ai', x: 270, y: 30, aiGarrison: { infantry: 10, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [39, 41, 14, 23], buildings: [], income: 2, support: { player: 15, ai: 75, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 41, name: 'Швеция', owner: 'ai', x: 260, y: -10, aiGarrison: { infantry: 15, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [40, 42], buildings: [], income: 2, support: { player: 15, ai: 75, werewolf: 10 }, population: 2000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 42, name: 'Норвегия', owner: 'ai', x: 180, y: -20, aiGarrison: { infantry: 10, archer: 2, cavalry: 1 }, siegeBy: null, neighbors: [41, 39], buildings: [], income: 2, support: { player: 15, ai: 75, werewolf: 10 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 43, name: 'Финляндия', owner: 'ai', x: 410, y: -10, aiGarrison: { infantry: 5, archer: 1, cavalry: 1 }, siegeBy: null, neighbors: [41, 35], buildings: [], income: 1, support: { player: 20, ai: 70, werewolf: 10 }, population: 1000, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 60 },
      { id: 44, name: 'Прибалтика', owner: 'ai', x: 460, y: 40, aiGarrison: { infantry: 8, archer: 2, cavalry: 2 }, siegeBy: null, neighbors: [35, 14, 23], buildings: [], income: 1, support: { player: 20, ai: 70, werewolf: 10 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'plains', terrainBonus: 0, loyalty: 70 },
      { id: 45, name: 'Северные земли', owner: 'werewolf', x: 560, y: -20, aiGarrison: { infantry: 10, archer: 2, cavalry: 3 }, siegeBy: null, neighbors: [22, 41, 35], buildings: [], income: 1, support: { player: 10, ai: 10, werewolf: 80 }, population: 1500, slaveIncome: 0, fortification: 0, terrain: 'forest', terrainBonus: 2, loyalty: 60 },
    ]
  };
}
let game: GameState = getDefaultGame();
const canvas = document.getElementById('game-canvas') as HTMLCanvasElement;
const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
const fogCanvas = document.getElementById('fog-canvas') as HTMLCanvasElement;
const fogCtx = fogCanvas.getContext('2d') as CanvasRenderingContext2D;

// ================= ТУМАН ВОЙНЫ =================
let fogParticles: {x: number; y: number; r: number; dx: number; dy: number}[] = [];
for(let i=0; i<35; i++) {
  fogParticles.push({
...
function showTutorialStep(step: number) {
  const modal = document.getElementById('tutorial-modal') as HTMLDivElement;
  const title = document.getElementById('tutorial-title') as HTMLHeadingElement;
  const desc = document.getElementById('tutorial-desc') as HTMLParagraphElement;
  const btn = document.getElementById('btn-tutorial-next') as HTMLButtonElement;
...
function restartGame() {
  ...
  document.querySelectorAll('.action-btn, .sub-btn').forEach(btn => (btn as HTMLButtonElement).disabled = false);
...
function getTotalTroops(armyObj: Army): number { return (armyObj.infantry || 0) + (armyObj.archer || 0) + (armyObj.cavalry || 0); }
function log(msg: string, type: string = 'system') {
...
function isNightTime(): boolean { return game.turn % 2 !== 0; }
...
function processLordsAfterBattle(isVictory: boolean, isAttacker: boolean) {
  if (!isAttacker) return;
  let aliveLords: { name: string; battles: number }[] = [];
  game.player.lords.forEach(l => {
...
function calcArmyPower(army: Army, isPlayer: boolean): number {
...
// ================= СБОР ДОХОДОВ И СЮЖЕТНЫЕ ФУНКЦИИ =================
...
function executeArmyBattle(attackerSide: 'player', targetSide: 'ai' | 'werewolf') {
  ...
  if (totalAtt > 0) { let attLeft = attackerLosses; units.forEach(type => { let count = attackerArmy[type as keyof Army] || 0; let take = Math.floor(attackerLosses * (count / totalAtt)); take = Math.min(take, count); (attackerArmy[type as keyof Army] as number) = Math.max(0, count - take); attLeft -= take; }); if (attLeft > 0 && totalAtt > 0) { attackerArmy.infantry = Math.max(0, attackerArmy.infantry - attLeft); } }
  if (totalDef > 0) { let defLeft = defenderLosses; units.forEach(type => { let count = targetArmy[type as keyof Army] || 0; let take = Math.floor(defenderLosses * (count / totalDef)); take = Math.min(take, count); (targetArmy[type as keyof Army] as number) = Math.max(0, count - take); defLeft -= take; }); if (defLeft > 0 && totalDef > 0) { targetArmy.infantry = Math.max(0, targetArmy.infantry - defLeft); } }
...
function checkCaptureRequirements(attackerSide: 'player' | 'ai' | 'werewolf', targetProv: Province): boolean {
...
function executeBattle(attackerSide: 'player' | 'ai' | 'werewolf', targetProv: Province) {
  ...
  let units = ['infantry', 'archer', 'cavalry'];
  if (totalAtt > 0) { let attLeft = attackerLosses; units.forEach(type => { let count = attackerArmy[type as keyof Army] || 0; let take = Math.floor(attackerLosses * (count / totalAtt)); take = Math.min(take, count); (attackerArmy[type as keyof Army] as number) = Math.max(0, count - take); attLeft -= take; }); if (attLeft > 0 && totalAtt > 0) { attackerArmy.infantry = Math.max(0, attackerArmy.infantry - attLeft); } }
  if (defenderGarrison && totalDef > 0) { let defLeft = defenderLosses; units.forEach(type => { let count = defenderGarrison[type as keyof Army] || 0; let take = Math.floor(defenderLosses * (count / totalDef)); take = Math.min(take, count); (defenderGarrison[type as keyof Army] as number) = Math.max(0, count - take); defLeft -= take; }); if (defLeft > 0 && totalDef > 0) { let inf = defenderGarrison.infantry || 0; defenderGarrison.infantry = Math.max(0, inf - defLeft); } }
...
function showSurrenderModal(prov: Province) {
  ...
  document.getElementById('btn-exterminate')!.onclick = () => { 
    game.player.gold += 1500; game.player.blood += 800; prov.loyalty = 5; prov.population -= 2000; 
    log(`💀 Истребление! +1500 золота, +800 крови.`, 'player'); r.textContent = "Кровь и пепел!"; closeSurrenderModal(); 
  };
  document.getElementById('btn-enslave')!.onclick = () => { 
    game.player.gold += 500; game.player.blood += 200; prov.loyalty = 50; prov.slaveIncome = 2; 
    if (!prov.playerGarrison) prov.playerGarrison = { infantry:0, archer:0, cavalry:0 }; prov.playerGarrison.infantry += 10; 
    log(`⛓️ Порабощение! +500 золота, +200 крови.`, 'player'); r.textContent = "Рабы пополнили гарнизон."; closeSurrenderModal(); 
  };
  document.getElementById('btn-convert')!.onclick = () => { 
    prov.loyalty += 20; if (!prov.playerGarrison) prov.playerGarrison = { infantry:0, archer:0, cavalry:0 }; prov.playerGarrison.infantry += 10; prov.population = Math.floor(prov.population * 1.2); 
...
function getTargetProvForAction(): Province | null {
...
// ================= ЭКОНОМИКА, СТРОЙКА И ЮНИТЫ =================
function buildStructure(type: string) {
  ...
  if (type === 'dark_temple') { cost = 20; } else if (type === 'barracks') { cost = 20; } else if (type === 'barracks_lv2') { cost = 50; lvl = 2; } else if (type === 'cemetery') { cost = 30; } else if (type === 'dungeon') { cost = 15; } else if (type === 'executions') { cost = 10; } else if (type === 'ball') { cost = 30; } else if (type === 'center') { cost = 25; } else if (type === 'citadel') { cost = 40; } else if (type === 'wall') { cost = 10; } else if (type === 'castle') { cost = 40; } else if (type === 'market') { cost = 20; } else { return log(`❌ Неизвестная постройка.`, 'system'); }
  ...
  else if (type === 'castle') { prov.fortification += 2; if (!prov.playerGarrison) prov.playerGarrison = { infantry:0, archer:0, cavalry:0 }; prov.playerGarrison.infantry += 20; prov.support.player += 5; log(`🏰 Построен Замок!`, 'player'); } 
...
function recruitTroops(type: string) {
  ...
  const u = { infantry: { cost: 10, count: 5 }, archer: { cost: 15, count: 5 }, cavalry: { cost: 20, count: 3 } }[type as 'infantry' | 'archer' | 'cavalry'];
  if (game.player.gold < u.cost) return log(`❌ Нужно ${u.cost} золота.`, 'system');
  game.player.gold -= u.cost;
  if (getTotalTroops(game.player.mobileArmy) === 0) { game.player.mobileArmy.location = prov.id; game.player.mobileArmy[type as keyof Army] = (game.player.mobileArmy[type as keyof Army] || 0) + u.count; log(`🧟 Возрождение армии!`, 'player'); } else if (prov.id === game.player.mobileArmy.location) { game.player.mobileArmy[type as keyof Army] = (game.player.mobileArmy[type as keyof Army] || 0) + u.count; log(`🧛 +${u.count} в мобильную армию.`, 'player'); } else { if (!prov.playerGarrison) prov.playerGarrison = { infantry:0, archer:0, cavalry:0 }; prov.playerGarrison[type as keyof Army] = (prov.playerGarrison[type as keyof Army] || 0) + u.count; log(`🛡️ +${u.count} в гарнизон ${prov.name}.`, 'player'); }
  game.player.ap -= 1; updateUI();
}
function moveTroops(amount: number, toGarrison: boolean = true) {
  if (!canAct()) return; const prov = game.provinces.find(p => p.id === game.player.mobileArmy.location); if (!prov || prov.owner !== 'player') return log('❌ Гарнизон управляется там, где стоит армия.', 'system'); if (!prov.playerGarrison) prov.playerGarrison = { infantry:0, archer:0, cavalry:0 }; let mobile = game.player.mobileArmy; let garrison = prov.playerGarrison; const unitTypes = ['infantry', 'archer', 'cavalry']; let totalMobile = getTotalTroops(mobile); if (totalMobile === 0 && toGarrison) return log('❌ В армии нет войск.', 'system'); if (getTotalTroops(garrison) === 0 && !toGarrison) return log('❌ В гарнизоне нет войск.', 'system'); let remaining = amount; let srcTotal = toGarrison ? totalMobile : getTotalTroops(garrison); if (srcTotal === 0) { log('❌ Ошибка перемещения.', 'system'); return; } unitTypes.forEach(type => { let srcCount = toGarrison ? (mobile[type as keyof Army] || 0) : (garrison[type as keyof Army] || 0); let take = Math.floor(amount * (srcCount / srcTotal)); take = Math.min(take, srcCount); take = Math.min(take, remaining); if (toGarrison) { (mobile[type as keyof Army] as number) = Math.max(0, (mobile[type as keyof Army] || 0) - take); garrison[type as keyof Army] = (garrison[type as keyof Army] || 0) + take; } else { (garrison[type as keyof Army] as number) = Math.max(0, (garrison[type as keyof Army] || 0) - take); mobile[type as keyof Army] = (mobile[type as keyof Army] || 0) + take; } remaining -= take; }); log(`${toGarrison ? '⬆️' : '⬇️'} ${amount} бойцов.`, 'player'); game.player.ap -= 1; updateUI();
}
...
function gameOver(winner: 'player' | 'ai') { if (game.gameOver) return; game.gameOver = true; document.querySelectorAll('.action-btn, .sub-btn').forEach(btn => (btn as HTMLButtonElement).disabled = true); document.getElementById('bg-layer')!.style.opacity = '0.8'; const modal = document.getElementById('gameover-modal')!; modal.style.display = 'flex'; saveGame(); }
function canAct(): boolean { return !game.gameOver && game.player.ap > 0 && !game.battleActive && !game.surrenderActive && !game.armyBattleActive; }
...
function startSunset() {
  const overlay = document.getElementById('sunset-overlay') as HTMLDivElement;
  if (!overlay) return;
...
function drawWeather() {
  if (game.weather.rain) {
    ...
    ctx.lineWidth = 1;
    for(let i=0; i<100; i++) { let x = Math.random() * canvas.width; let y = Math.random() * canvas.height; ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x+4, y+15); ctx.stroke(); }
  }
  if (game.weather.lightning) {
    for(let i=0; i<3; i++) {
      let startX = 100 + Math.random() * 500; let startY = 20;
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.8)'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(startX, startY);
      let prevX = startX, prevY = startY;
      for(let j=0; j<6; j++) { let nextX = prevX + (Math.random() - 0.5) * 70; let nextY = prevY + 20 + Math.random() * 40; ctx.lineTo(nextX, nextY); prevX = nextX; prevY = nextY; }
      ctx.stroke();
    }
  }
}
      ...
      if (enemies.length > 0) {
        ...
        if (f.ownerType === 'werewolf' && !isNightTime()) { } else {
          if (target.siegeBy === null && getTotalTroops(f.obj.mobileArmy) > 0) {
            ...
            units.forEach(type => { let c = attArmy[type as keyof Army] || 0; let take = Math.floor(aLoss * (c / (totalAtt + 1))); take = Math.min(take, c); (attArmy[type as keyof Army] as number) = Math.max(0, c - take); });
            let totalDef = getTotalTroops(defGar || {}) + target.fortification * 5 + (target.terrainBonus || 0);
            units.forEach(type => { let c = defGar && defGar[type as keyof Army] || 0; let take = Math.floor(dLoss * (c / (totalDef + 1))); take = Math.min(take, c); if (defGar) (defGar[type as keyof Army] as number) = Math.max(0, c - take); });
...
function drawMap() {
  ...
  const playerVisible: number[] = []; game.provinces.forEach(p => { if (p.owner === 'player') { playerVisible.push(p.id); p.neighbors.forEach(n => playerVisible.push(n)); } });
  game.provinces.forEach(p => {
    ...
    ctx.fillStyle = gradient; ctx.strokeStyle = p.owner === 'player' ? '#4a5b9a' : (p.owner === 'ai' ? '#808ca0' : (p.owner === 'werewolf' ? '#3d4d3d' : '#2a2a2a'));
    ctx.fill(); ctx.stroke();
...
function updateUI() {
  ensureArmyLocation();
  document.getElementById('turn-counter')!.textContent = game.turn.toString();
  document.getElementById('day-counter')!.textContent = game.day.toString();
  document.getElementById('ap-counter')!.textContent = `${game.player.ap}/${game.player.maxAp}`;
  document.getElementById('blood-counter')!.textContent = game.player.blood.toString();
  document.getElementById('gold-counter')!.textContent = game.player.gold.toString();
  document.getElementById('elite-counter')!.textContent = game.player.lords.length.toString();
  const faithFill = document.getElementById('faith-bar-fill') as HTMLDivElement; let faithPct = Math.min(100, game.ai.faith);
  ...
  const assaultBtn = document.getElementById('btn-assault') as HTMLButtonElement;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.dropdown-toggle').forEach(btn => {
    (btn as HTMLButtonElement).addEventListener('click', function(e) {
      e.stopPropagation();
      const content = this.parentElement!.querySelector('.dropdown-content') as HTMLDivElement;
  ...
  document.getElementById('btn-music-toggle')!.addEventListener('click', () => {
    const bgm = document.getElementById('bgm') as HTMLAudioElement;
    if (bgm.paused) { bgm.volume = 0.4; bgm.play(); document.getElementById('btn-music-toggle')!.textContent = "🎵 ВЫКЛ"; } else { bgm.pause(); document.getElementById('btn-music-toggle')!.textContent = "🎵 ВКЛ"; }
  });
  ...
  document.getElementById('dip-truce-ai')!.addEventListener('click', () => { if (game.player.gold < 30) return log('❌ Не хватает золота.', 'system'); if (game.player.truceTurnsAI > 0) return log('⛔ Перемирие уже активно.', 'system'); game.player.gold -= 30; game.player.truceTurnsAI = 2; log(`🕊️ Перемирие с Ватиканом на 2 хода!`, 'player'); document.getElementById('diplomacy-result')!.textContent = "Перемирие заключено!"; setTimeout(closeDiplomacy, 1000); updateUI(); });
  document.getElementById('dip-truce-wolf')!.addEventListener('click', () => { if (game.player.gold < 30) return log('❌ Не хватает золота.', 'system'); if (game.player.truceTurnsWolf > 0) return log('⛔ Перемирие уже активно.', 'system'); game.player.gold -= 30; game.player.truceTurnsWolf = 2; log(`🕊️ Перемирие с Оборотнями на 2 хода!`, 'player'); document.getElementById('diplomacy-result')!.textContent = "Перемирие заключено!"; setTimeout(closeDiplomacy, 1000); updateUI(); });
  document.getElementById('dip-alliance')!.addEventListener('click', () => { if (game.player.gold < 50) return log('❌ Не хватает золота.', 'system'); if (game.player.allianceWithAI) return log('⛔ Союз уже активен.', 'system'); game.player.gold -= 50; game.player.allianceWithAI = true; log(`⚔️ Заключен союз с Ватиканом против Оборотней!`, 'player'); document.getElementById('diplomacy-result')!.textContent = "Союз заключен!"; setTimeout(closeDiplomacy, 1000); updateUI(); });
  document.getElementById('mkt-gold-to-blood')!.addEventListener('click', () => {
    if (game.player.marketUsed) return log('⛔ Рынок уже использован.', 'system');
    ...
    log(`⚖️ Обмен: 10 Золота → 8 Крови.`, 'player'); document.getElementById('market-result')!.textContent = "Обмен успешен!"; setTimeout(closeMarket, 1000); updateUI();
  });
  document.getElementById('mkt-blood-to-gold')!.addEventListener('click', () => {
    if (game.player.marketUsed) return log('⛔ Рынок уже использован.', 'system');
    if (game.player.blood < 10) return log('❌ Нужно 10 крови.', 'system');
    game.player.blood -= 10; game.player.gold += 8; game.player.marketUsed = true;
    log(`⚖️ Обмен: 10 Крови → 8 Золота.`, 'player'); document.getElementById('market-result')!.textContent = "Обмен успешен!"; setTimeout(closeMarket, 1000); updateUI();
  });
  document.getElementById('tech-reform')!.addEventListener('click', () => {
    ...
    log(`⚔️ Исследована "Военная реформа"! +10% к мощи армии.`, 'player'); document.getElementById('tech-result')!.textContent = "Технология изучена!"; setTimeout(closeTech, 1000); updateUI();
  });
  document.getElementById('tech-necro')!.addEventListener('click', () => {
    ...
    log(`💀 Исследована "Некромантия"! Убитые враги дают кровь.`, 'player'); document.getElementById('tech-result')!.textContent = "Технология изучена!"; setTimeout(closeTech, 1000); updateUI();
  });
  document.getElementById('tech-trade')!.addEventListener('click', () => {
    ...
    log(`📜 Исследованы "Торговые пути"! Рынок можно использовать 2 раза.`, 'player'); document.getElementById('tech-result')!.textContent = "Технология изучена!"; setTimeout(closeTech, 1000); updateUI();
  });
  ...
  document.getElementById('btn-attack-army')!.addEventListener('click', () => {
    ...
    if(targetProv && targetProv.owner !== 'player') { game.player.mobileArmy.location = targetProv.id; log(`🧛 Ваша армия выдвинулась на перехват врага в ${targetProv.name}.`, 'player'); }
  ...
  document.querySelectorAll('[data-lore]').forEach(btn => {
    (btn as HTMLButtonElement).addEventListener('mouseenter', (e) => {
      const loreKey = (e.target as HTMLElement).getAttribute('data-lore');
      ...
      if (loreText) {
        ...
        loreTooltip.style.left = (e as MouseEvent).pageX + 10 + 'px';
        loreTooltip.style.top = (e as MouseEvent).pageY + 10 + 'px';
      }
    });
    (btn as HTMLButtonElement).addEventListener('mouseleave', () => { document.getElementById('lore-tooltip')!.style.display = 'none'; });
  });
